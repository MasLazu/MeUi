...
public class GetUserResourceActionsQueryHandler : ICommandHandler<GetResourceActionsQuery, IEnumerable<ResourceActionDto>>
{
    private readonly IAuthorizationCoreRepository<ResourceAction> _resourceActionRepository;

    public GetUserResourceActionsQueryHandler(IAuthorizationCoreRepository<ResourceAction> resourceActionRepository)
    {
        _resourceActionRepository = resourceActionRepository;
    }

    public async Task<IEnumerable<ResourceActionDto>> ExecuteAsync(GetResourceActionsQuery command, CancellationToken ct)
    {
        List<ResourceAction> resourceActions = await _resourceActionRepository.ListAsync(new ResourceActionWithResourceAndActionSpesification(), ct);

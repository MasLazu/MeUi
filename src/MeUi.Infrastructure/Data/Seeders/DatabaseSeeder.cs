...
public class DatabaseSeeder
{
    ...
    {
        List<(string Action, string Resource)> permissions = ScanPermissionsFromAssemblies();
        var resources = permissions.Select(p => p.Resource).Distinct().ToList();

        foreach (string? resourceCode in resources)
        {
    ...
    private async Task SeedActionsAsync(CancellationToken ct)
    {
        ...
        foreach (string? actionCode in actions)
        {
            ...
            foreach (Assembly? assembly in assemblies)
            {
                try
                {
                    ...
                    foreach (Type? type in types)
                    {
                        ...
                        foreach (RequirePermissionAttribute attribute in attributes)
                        {
                            ...
                            {
                                if (TryParsePermission(permissionString, out string? action, out string? resource))
                                {
    ...
    private async Task SeedPagePermissionsAsync(CancellationToken ct)
    {
        ...
        foreach ((Guid? pageId, Guid? permissionId) in pagePermissions)
        {

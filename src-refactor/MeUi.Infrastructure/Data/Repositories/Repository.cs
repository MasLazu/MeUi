...
public class Repository<T> : IRepository<T> where T : BaseEntity
{
    ...
    {
        return await _dbSet.FirstOrDefaultAsync(x => x.Id == id && x.DeletedAt == null, ct);
    }

    public virtual async Task<IEnumerable<T>> GetAllAsync(CancellationToken ct = default)
    {
        return await _dbSet.Where(x => x.DeletedAt == null).ToListAsync(ct);
    }

    public virtual async Task<IEnumerable<T>> FindAsync(Expression<Func<T, bool>> predicate, CancellationToken ct = default)
    {
        return await _dbSet.Where(x => x.DeletedAt == null).Where(predicate).ToListAsync(ct);
    }

    public virtual async Task<T?> FirstOrDefaultAsync(Expression<Func<T, bool>> predicate, CancellationToken ct = default)
    {
        return await _dbSet.Where(x => x.DeletedAt == null).FirstOrDefaultAsync(predicate, ct);
    }
    ...
    {
        var query = _dbSet.Where(x => x.DeletedAt == null);
